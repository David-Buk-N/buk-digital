...
export default function RadialOrbitalTimeline({
  ...
  const getStatusStyles = (status: TimelineItem["status"]): string => {
    switch (status) {
      ...
      case "pending":
        return "text-foreground bg-transparent border-foreground/40";
      default:
        return "text-foreground bg-transparent border-foreground/40";
    }
  };
  ...
  return (
    <div
      className="w-full h-[36rem] flex flex-col items-center justify-center overflow-hidden"
      ref={containerRef}
    ...
    >
      <div className="relative w-full max-w-4xl h-full flex items-center justify-center">
        ...
        >
          <div className="absolute w-16 h-16 rounded-full bg-gradient-to-br from-primary via-primary/80 to-primary/50 animate-pulse flex items-center justify-center z-10">
          ...
          {timelineData.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                {isExpanded && (
                  <Card className="absolute top-20 left-1/2 -translate-x-1/2 w-64 bg-card/90 backdrop-blur-lg border-border shadow-xl shadow-primary/10 overflow-visible">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {item.relatedIds.map((relatedId) => {
                              ...
                              return (
                                <button
                                  key={relatedId}
                                  className="inline-flex items-center h-6 px-2 rounded border border-border text-xs text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors"
                                  onClick={(e) => {
                                    e.stopPropagation();
