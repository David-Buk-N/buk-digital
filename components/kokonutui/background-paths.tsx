...
function generateAestheticPath(
  ...
  // Round to 2 decimals: full-precision floats can differ in their last
  // digits between the server and client bundles, which triggers a
  // hydration mismatch on the `d` attribute.
  const r = (n: number) => Math.round(n * 100) / 100;

  const pathCommands = points.map((point: Point, i: number) => {
    if (i === 0) return `M ${r(point.x)} ${r(point.y)}`;
    ...
    const cp2y = point.y;
    return `C ${r(cp1x)} ${r(cp1y)}, ${r(cp2x)} ${r(cp2y)}, ${r(point.x)} ${r(point.y)}`;
  });

  return pathCommands.join(" ");
}

// Deterministic IDs — random values here differ between server and client
